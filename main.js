...
const App = {
    ...
    handleSubmitForm: function() {
        if((name1.value.length >= 3 && name1.value.length < 25) && (name2.value.length >= 3 && name2.value.length < 25)) {
            if(App.containsNums([name1.value, name2.value])) alert('Vui lòng nhập đúng tên!');
            else {
                        ...
                        let interval = setInterval(() => {
                            if(currentPercent < result.percent) {
                                ...
                                if(result.percent != 100) {
                                    percentModal.innerText = result.percent + Math.floor(Math.random() * 10) + '%'
                                }
